...
class Eval(object):
    ...
    def __init__(self, model):
        ...
        vld_x, _ = zip(*sorted(vld_data.items()))


        self.vld_gen = Datagen_tree(vld_x, config.eval_batch_size, code_w2i,  train=False)
    ...
    def eval_one_batch(self, x, batch, batch_size, criterion):
        """
        evaluate one batch
        :param batch:
        :param batch_size:
    ...
    def eval_iter(self):
        ...
        t = self.vld_gen(3047)
        for (index_batch, batch), (x) in zip(enumerate(self.dataloader), t):
            batch_size = batch[0].shape[1]

            loss = self.eval_one_batch(x, batch, batch_size, criterion=criterion)
...
class Test(object):
    ...
    def __init__(self, model):
        ...
        self.tst_gen = Datagen_tree(tst_x,  config.test_batch_size, code_w2i,  train=False)
    ...
    def test_one_batch(self, x, batch, batch_size):
        """
        ...
        with torch.no_grad():
            ...
            source_outputs, code_outputs, ast_outputs, decoder_hidden = \
                self.model(x, batch, batch_size, self.nl_vocab, is_test=True)
    ...
    def test_iter(self):
        ...
        for (index_batch, batch), (x) in zip(enumerate(self.dataloader), t):
            ...
            batch_size = batch[0].shape[1]
            references, candidates, s_blue_score, meteor_score = self.test_one_batch(x, batch, batch_size)
        ...
        for index_batch in range(batch_size):
            ...
            for step in range(config.max_decode_steps):
                ...
                decoder_outputs, new_decoder_hidden, \
                    source_attn_weights, code_attn_weights, ast_attn_weights = self.model.decoder(inputs=feed_inputs,
