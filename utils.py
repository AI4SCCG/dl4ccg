...
def filter_data(codes, asts, nls):
    ...
    """
    assert len(codes) == len(asts)  #assert 断言语句，为真则程序往下运行，否则报错
...
def unsort_collate_fn(batch, source_vocab, code_vocab, ast_vocab, nl_vocab, raw_nl=False):
    """
    process the batch without sorting  处理批次而不排序
    ...
    return source_batch, source_seq_lens, \
...
class Datagen_tree:
    def __init__(self, trn_x, batch_size, code_dic, train=None, binary=False):
        self.X = trn_x
...
def get_nums(roots):
    ...
    res = [torch.tensor(seq + [-1] * (max_len - len(seq))) for seq in res]
    res = torch.stack(res, dim=0)

    res = torch.nn.utils.rnn.pad_sequence(res, batch_first=True, padding_value=-1)
